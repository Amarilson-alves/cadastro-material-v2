...
import { Trash2, Edit2, Shield, Loader2, HardHat, Search, Eye, EyeOff, Plus, Users, Crown } from 'lucide-react';
...
function RoleIcon({ role }: { role: string }) {
  if (role === 'master') return <Crown className="h-3.5 w-3.5 text-amber-500" />;
  if (role === 'staff')  return <Shield className="h-3.5 w-3.5 text-green-500" />;
  return <HardHat className="h-3.5 w-3.5 text-orange-500" />;
}

export default function GerenciarEquipe() {
  const { perfil: perfilLogado, isMaster } = useAuth();
  ...
  const mutationEditar = useMutation({
    mutationFn: async (dados: { id: string; nome: string; role: string; senha?: string }) => {
      const { data, error } = await supabase.functions.invoke('gerenciar-usuarios', {
        body: { acao: 'editar', userId: dados.id, nome: dados.nome, role: dados.role, senha: dados.senha },
      });
      if (error) throw new Error(error.message);
      if (data?.error) throw new Error(data.error);
    },
  ...
  // Admin só vê técnico. Master vê técnico + staff + master
  const opcoesRole = isMaster
    ? [
        { value: 'tecnico', label: 'Técnico (Campo)' },
  ...
  // Regras de quem pode editar/deletar quem
  const podeEditar = (alvo: Perfil) => {
    if (isMaster) return true;
    // Admin não pode editar outros admins nem masters
    return alvo.role === 'tecnico';
  };

  const podeDeletar = (alvo: Perfil) => {
    if (alvo.id === perfilLogado?.id) return false; // Não pode se auto-deletar
    if (isMaster) return true;
    return alvo.role === 'tecnico';
  };
    ...
    <div className="space-y-5 animate-fade-in">
      ...
      <div className="card p-6">
        <div className="flex items-center gap-2 mb-5">
          ...
          <h2 className="text-sm font-semibold text-slate-600">
            {isMaster ? 'Adicionar usuário (qualquer role)' : 'Adicionar técnico'}
          </h2>
        </div>
        ...
        <form onSubmit={handleSubmit} className="grid grid-cols-1 sm:grid-cols-5 gap-3">
          ...
          <select value={form.funcao} onChange={e => setForm({ ...form, funcao: e.target.value })} className={ic}>
            {opcoesRole.map(o => <option key={o.value} value={o.value}>{o.label}</option>)}
          </select>
      ...
      <div className="card p-6">
        ...
        {isLoading && buscaUsuario && (
          <div className="flex justify-center py-6"><Loader2 className="h-5 w-5 text-gray-400 animate-spin" /></div>
        )}
              ...
              <div key={membro.id}>
                {editando?.id === membro.id ? (
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-6 gap-2 items-center p-3 rounded-xl border border-gray-100 bg-gray-50/50 hover:bg-white hover:border-gray-200 hover:shadow-sm transition-smooth text-sm">
                    ...
                    <span className="font-mono text-[11px] bg-gray-100 text-gray-600 px-2 py-1 rounded-lg w-fit">{membro.matricula}</span>
                    <span className="flex items-center gap-1.5 text-xs font-medium text-gray-600">
                      <RoleIcon role={membro.role} />
                      {roleLabel(membro.role)}
                    </span>
                    <span className="text-xs text-green-700 bg-green-50 px-2 py-1 rounded-lg w-fit font-medium">Ativo</span>
                    <div className="flex gap-1.5 justify-end">
                      {podeEditar(membro) && (
                        <button onClick={() => setEditando(membro)}
                      ...
                      {podeDeletar(membro) && (
                        <button onClick={() => {
                          if (window.confirm(`Remover permanentemente o acesso de ${membro.nome}?`))
...
function EditarUsuarioForm({ usuario, isMaster, onSalvar, onCancelar, salvando }: {
  usuario: Perfil;
  isMaster: boolean;
    ...
    <div className="border border-blue-200 rounded-2xl p-4 bg-blue-50/40 space-y-3 animate-fade-in my-1">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-4 gap-3">
        <input value={dados.nome} onChange={e => setDados({ ...dados, nome: e.target.value })}
          className={`${ic} sm:col-span-2`} placeholder="Nome completo" />
        <select value={dados.role} onChange={e => setDados({ ...dados, role: e.target.value })}
          className={ic}>
          {opcoesRole.map(o => <option key={o.value} value={o.value}>{o.label}</option>)}
        </select>
        <div className="relative">
      ...
      <div className="flex gap-2">
        <button onClick={() => onSalvar({ id: usuario.id, nome: dados.nome, role: dados.role, senha: dados.senha })}
          disabled={salvando}
